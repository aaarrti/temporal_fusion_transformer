...
from temporal_fusion_transformer.src.utils import map_dict, filter_dict

if TYPE_CHECKING:
    from temporal_fusion_transformer.src.modeling import TemporalFusionTransformer


TargetScaler = Callable[[str, np.ndarray], np.ndarray]
...
class InferenceService:
    ...
    def plot_predictions_for_entity(
        ...
        if logits.shape[1] != num_future_steps:
            raise ValueError(
                f"Expected model to return {num_future_steps} timestamps, but received {logits.shape[1]}"
            )
        ...
        logits = np.reshape(
            logits, (batch_size, num_future_steps, -1, len(self.model.quantiles))
        )
