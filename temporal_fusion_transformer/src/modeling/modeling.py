...
from temporal_fusion_transformer.src.utils import enumerate_v2, log_before, zip_v2
...
def TemporalFusionTransformer(
    ...
    **kwargs,
    ...
    outputs = ops.stack(
        [
            layers.TimeDistributed(layers.Dense(num_quantiles), name=f"out_{i}")(
...
def apply_tft_embeddings(
    ...
    *,
    ...
    input_known_categorical_idx: Sequence[int],
    hidden_layer_size: int,
) -> tuple[keras.KerasTensor | None, keras.KerasTensor, keras.KerasTensor | None]:
...
def linear_layer(
    ...
    if use_time_distributed:
        linear = layers.TimeDistributed(linear, name=f"{linear.name}.time_distributed")
    return linear
...
def apply_gating_layer(
    ...
    if use_time_distributed:
        activation_layer = layers.TimeDistributed(
            layers.Dense(hidden_layer_size, activation=activation), name=f"x.{x.name}.activation"
        )(x)
        gated_layer = layers.TimeDistributed(
            layers.Dense(hidden_layer_size, activation="sigmoid"), name=f"x.{x.name}.gate"
        )(x)
    else:
...
@log_before(ignore_argnames=("x",), logger=print)
def gated_residual_network(
    ...
    *,
    ...
    if output_size is None:
        ...
        if use_time_distributed:
            linear = layers.TimeDistributed(linear, name=f"here.{linear.name}.time")
        skip = linear(x)
