...
class InputEmbedding(layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        self.static_embeddings = [
            layers.Embedding(size, hidden_layer_size, name=f"static_{i}")
            for i, size in enumerate(static_categories_sizes)
        ]

        self.known_categorical_inputs_embeddings = [
            layers.Embedding(size, hidden_layer_size, name=f"known_categorical_{i}")
            for i, size in enumerate(known_categories_sizes)
        ]
        ...
        self.known_real_projection = [
            Linear(
                hidden_layer_size, use_time_distributed=True, use_bias=False, name=f"known_real_{i}"
            )
            for i in input_known_real_idx
        ]

        self.observed_projection = [
            Linear(
                hidden_layer_size, use_time_distributed=True, use_bias=False, name=f"observed_{i}"
            )
            for i in input_observed_idx
        ]
    ...
    def call(self, inputs, *args, **kwargs):
        # ops.fori_loop(0, len(self.input_static_idx))
...
class GatedResidualNetwork(layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        if output_size is None:
            ...
            skip = layers.Identity(name="skip")
        else:
            skip = Linear(
                output_size,
                use_time_distributed=use_time_distributed,
        ...
        self.activation = layers.Activation("elu", name="activation")
        self.post_activation = Linear(
            hidden_layer_size,
            use_time_distributed=use_time_distributed,
            activation=None,
            kernel_regularizer=kernel_regularizer,
        ...
        self.add_and_norm = AddAndNorm(name="add_and_norm")
    ...
    def call(self, x: tf.Tensor, context: tf.Tensor | None = None, **kwargs):
        with named_scope(self.name):
            ...
            hidden = self.pre_activation(x)
            hidden = self.activation(hidden)
            hidden = self.post_activation(hidden)
...
class GatedResidualNetworkWithContext(GatedResidualNetwork):
    ...
    def call(self, x: ContextInput, **kwargs):
        with named_scope(self.name):
            ...
            # Apply feedforward network
            hidden = self.pre_activation(inputs)
    ...
    def call(self, x: ContextInput, **kwargs):
        with named_scope(self.name):
            ...
            mlp_outputs, static_gate = self.grn(
                {"input": flatten, "context": expanded_static_context}
            )

            sparse_weights = self.activation(mlp_outputs)
