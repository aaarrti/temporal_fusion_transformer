...
def download_from_url(url, output_path):
    ...
    print(f"Pulling data from {url} to {output_path}")
...
def unzip(zip_path, output_file, data_folder):
    ...
    if not os.path.exists(output_file):
        raise ValueError(f"Error in unzipping process! {output_file} not found.")
...
def download_volatility(config):
    ...
    print(f"Completed formatting, saving to {output_file}")
...
def download_traffic(config):
    ...
    def read_single_list(filename):
        ...
        with open(os.path.join(data_folder, filename)) as dat:
    ...
    def read_matrix(filename):
        ...
        with open(os.path.join(data_folder, filename)) as dat:
            ...
            for i, line in enumerate(lines):
                if (i + 1) % 50 == 0:
                    print(f"Completed {i + 1} of {len(lines)} rows for {filename}")
    ...
    labels = [f"traj_{i}" for i in read_single_list("stations_list")]
...
def process_favorita(config):
    ...
    for traj_id, raw_sub_df in temporal.groupby("traj_id"):
        ...
        sub_df[["store_nbr", "item_nbr", "onpromotion"]] = sub_df[["store_nbr", "item_nbr", "onpromotion"]].fillna(
            method="ffill"
        )
    ...
    temporal = temporal.merge(transactions, left_on=["date", "store_nbr"], right_on=["date", "store_nbr"], how="left")
    ...
    temporal["national_hol"] = temporal.merge(national_holidays, left_on=["date"], right_on=["date"], how="left")[
        "description"
    ].fillna("")
    temporal["regional_hol"] = temporal.merge(
        regional_holidays, left_on=["state", "date"], right_on=["locale_name", "date"], how="left"
    )["description"].fillna("")
    temporal["local_hol"] = temporal.merge(
        local_holidays, left_on=["city", "date"], right_on=["locale_name", "date"], how="left"
    )["description"].fillna("")

    temporal.sort_values("unique_id", inplace=True)

    print(f"Saving processed file to {config.data_csv_path}")
...
def main(expt_name, force_download, output_folder):
    ...
    if os.path.exists(expt_config.data_csv_path) and not force_download:
        print(f"Data has been processed for {expt_name}. Skipping download...")
        sys.exit(0)
    else:
    ...
    if expt_name not in download_functions:
        raise ValueError(f"Unrecongised experiment! name={expt_name}")

    download_function = download_functions[expt_name]

    # Run data download
    print(f"Getting {expt_name} data...")
...
if __name__ == "__main__":
    ...
    def get_args():
        ...
        parser.add_argument(
            "output_folder", metavar="f", type=str, nargs="?", default=".", help="Path to folder for data download"
        )
        parser.add_argument(
            "force_download",
            metavar="r",
