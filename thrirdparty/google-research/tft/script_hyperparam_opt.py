...
def main(expt_name, use_gpu, restart_opt, model_folder, hyperparam_iterations, data_csv_path, data_formatter):
    ...
    if not isinstance(data_formatter, data_formatters.base.GenericDataFormatter):
        raise ValueError("Data formatters should inherit from" + f"AbstractDataFormatter! Type={type(data_formatter)}")
    ...
    print(f"### Running hyperparameter optimization for {expt_name} ###")
    ...
    while len(opt_manager.results.columns) < hyperparam_iterations:
        print(
            "# Running hyperparam optimisation {} of {} for {}".format(
        ...
        with tf.Graph().as_default(), tf.compat.v1.Session(config=tf_config) as sess:
            ...
            if np.allclose(val_loss, 0.0) or np.isnan(val_loss):
