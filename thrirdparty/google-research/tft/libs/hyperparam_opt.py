...
class HyperparamOptManager:
    ...
    def _check_params(self, params):
        ...
        if invalid_fields:
            raise ValueError(f"Invalid Fields Found {invalid_fields} - Valid ones are {valid_fields}")
        if missing_fields:
            raise ValueError(f"Missing Fields Found {missing_fields} - Valid ones are {valid_fields}")
...
class DistributedHyperparamOptManager(HyperparamOptManager):
    ...
    def __init__(
        ...
        self.serialised_ranges_path = os.path.join(serialised_ranges_folder, f"ranges_{search_iterations}.csv")
