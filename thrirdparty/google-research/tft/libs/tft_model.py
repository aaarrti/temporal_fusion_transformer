...
def apply_mlp(
    ...
    """
    if use_time_distributed:
        hidden = tf_keras.layers.TimeDistributed(
            tf_keras.layers.Dense(hidden_size, activation=hidden_activation)
        )(inputs)
        return tf_keras.layers.TimeDistributed(
            tf_keras.layers.Dense(output_size, activation=output_activation)
        )(hidden)
    else:
...
def apply_gating_layer(
    ...
    if use_time_distributed:
        ...
        gated_layer = tf_keras.layers.TimeDistributed(
            tf_keras.layers.Dense(hidden_layer_size, activation="sigmoid")
        )(x)
    else:
...
class TemporalFusionTransformer:
    ...
    def get_tft_embeddings(self, all_inputs):
        ...
        known_combined_layer = tf_keras.backend.stack(
    ...
    def _build_base_graph(self):
        ...
        unknown_inputs, known_combined_layer, obs_inputs, static_inputs = self.get_tft_embeddings(
            all_inputs
        )
    ...
    def build_model(self):
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            outputs = tf_keras.layers.TimeDistributed(
                tf_keras.layers.Dense(self.output_size * len(self.quantiles))
    ...
    def fit(self, train_df=None, valid_df=None):
        ...
        callbacks = [
            tf_keras.callbacks.EarlyStopping(
                monitor="val_loss", patience=self.early_stopping_patience, min_delta=1e-4
            ),
    ...
    def predict(self, df, return_targets=False):
        ...
        def format_outputs(prediction):
            ...
            flat_prediction = pd.DataFrame(
                prediction[:, :, 0],
                columns=[f"t+{i}" for i in range(self.time_steps - self.num_encoder_steps)],
            )
