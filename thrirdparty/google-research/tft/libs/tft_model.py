...
# Layer definitions.
concat = tf_keras.backend.concatenate
stack = tf_keras.backend.stack
K = tf_keras.backend
Add = tf_keras.layers.Add
LayerNorm = tf_keras.layers.LayerNormalization
Dense = tf_keras.layers.Dense
Multiply = tf_keras.layers.Multiply
Dropout = tf_keras.layers.Dropout
Activation = tf_keras.layers.Activation
Lambda = tf_keras.layers.Lambda
...
def linear_layer(size, activation=None, use_time_distributed=False, use_bias=True):
    ...
    """
    linear = tf_keras.layers.Dense(size, activation=activation, use_bias=use_bias)
    if use_time_distributed:
        linear = tf_keras.layers.TimeDistributed(linear)
    return linear
...
def apply_mlp(
    ...
    """
    if use_time_distributed:
        hidden = tf_keras.layers.TimeDistributed(tf_keras.layers.Dense(hidden_size, activation=hidden_activation))(
            inputs
        )
        return tf_keras.layers.TimeDistributed(tf_keras.layers.Dense(output_size, activation=output_activation))(hidden)
    else:
        hidden = tf_keras.layers.Dense(hidden_size, activation=hidden_activation)(inputs)
        return tf_keras.layers.Dense(output_size, activation=output_activation)(hidden)
...
def apply_gating_layer(x, hidden_layer_size, dropout_rate=None, use_time_distributed=True, activation=None):
    ...
    if use_time_distributed:
        activation_layer = tf_keras.layers.TimeDistributed(
            tf_keras.layers.Dense(hidden_layer_size, activation=activation)
        )(x)
        gated_layer = tf_keras.layers.TimeDistributed(tf_keras.layers.Dense(hidden_layer_size, activation="sigmoid"))(x)
    else:
        activation_layer = tf_keras.layers.Dense(hidden_layer_size, activation=activation)(x)
        gated_layer = tf_keras.layers.Dense(hidden_layer_size, activation="sigmoid")(x)

    return tf_keras.layers.Multiply()([activation_layer, gated_layer]), gated_layer
...
def gated_residual_network(
    ...
    hidden = tf_keras.layers.Activation("elu")(hidden)
...
class TemporalFusionTransformer:
    ...
    def get_tft_embeddings(self, all_inputs):
        ...
        for i in range(num_categorical_variables):
            embedding = tf_keras.Sequential(
                [
                    tf_keras.layers.InputLayer([time_steps]),
        ...
        if self._static_input_loc:
            static_inputs = [
                tf_keras.layers.Dense(self.hidden_layer_size)(regular_inputs[:, 0, i : i + 1])
            ...
            static_inputs = tf_keras.backend.stack(static_inputs, axis=1)
        ...
        def convert_real_to_embedding(x):
            ...
            return tf_keras.layers.TimeDistributed(tf_keras.layers.Dense(self.hidden_layer_size))(x)

        # Targets
        obs_inputs = tf_keras.backend.stack(
        ...
        if unknown_inputs + wired_embeddings:
            unknown_inputs = tf_keras.backend.stack(unknown_inputs + wired_embeddings, axis=-1)
        else:
            unknown_inputs = None
        ...
        known_combined_layer = tf_keras.backend.stack(known_regular_inputs + known_categorical_inputs, axis=-1)
    ...
    def _batch_sampled_data(self, data, max_samples):
        ...
        if max_samples > 0 and len(valid_sampling_locations) > max_samples:
            ...
            print(f"Max samples={max_samples} exceeds # available segments={len(valid_sampling_locations)}")
    ...
    def _build_base_graph(self):
        ...
        # Inputs.
        all_inputs = tf_keras.layers.Input(
            shape=(
                time_steps,
                combined_input_size,
            )
        ...
        def static_combine_and_mask(embedding):
            ...
            flatten = tf_keras.layers.Flatten()(embedding)
            ...
            sparse_weights = tf_keras.layers.Activation("softmax")(mlp_outputs)
            ...
            combined = tf_keras.layers.Multiply()([sparse_weights, transformed_embedding])
    ...
    def build_model(self):
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            outputs = tf_keras.layers.TimeDistributed(tf_keras.layers.Dense(self.output_size * len(self.quantiles)))(
                transformer_layer[Ellipsis, self.num_encoder_steps :, :]
            )

            self._attention_components = attention_components

            adam = tf_keras.optimizers.legacy.Adam(lr=self.learning_rate, clipnorm=self.max_gradient_norm)

            model = tf_keras.Model(inputs=all_inputs, outputs=outputs)
            ...
            class QuantileLossCalculator:
                """Computes the combined tau loss for prespecified quantiles.
                ...
                def quantile_loss(self, a, b):
                    """Returns tau loss for specified quantiles.
    ...
    def fit(self, train_df=None, valid_df=None):
        ...
        callbacks = [
            tf_keras.callbacks.EarlyStopping(monitor="val_loss", patience=self.early_stopping_patience, min_delta=1e-4),
        ...
        if os.path.exists(tmp_checkpont):
            ...
            print(f"Cannot load from {self._temp_folder}, skipping ...")
    ...
    def predict(self, df, return_targets=False):
        ...
        # Extract predictions for each tau into different entries
        process_map = {
            f"p{int(q * 100)}": combined[Ellipsis, i * self.output_size : (i + 1) * self.output_size]
