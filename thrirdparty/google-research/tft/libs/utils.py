...
def tensorflow_quantile_loss(y, y_pred, quantile):
    """Computes tau loss for tensorflow.

    Standard tau loss as defined in the "Training Procedure" section of
    the main TFT paper
    ...
    if quantile < 0 or quantile > 1:
        raise ValueError(f"Illegal tau value={quantile}! Values should be between 0 and 1.")
...
def numpy_normalised_quantile_loss(y, y_pred, quantile):
    """Computes normalised tau loss for numpy arrays.
...
def save(tf_session, model_folder, cp_name, scope=None):
    ...
    save_path = saver.save(tf_session, os.path.join(model_folder, f"{cp_name}.ckpt"))
...
def load(tf_session, model_folder, cp_name, scope=None, verbose=False):
    ...
    """
    # Load model proper
    load_path = os.path.join(model_folder, f"{cp_name}.ckpt")

    print(f"Loading model from {load_path}")

    print_weights_in_checkpoint(model_folder, cp_name)

    initial_vars = {v.name for v in tf.compat.v1.get_default_graph().as_graph_def().node}
    ...
    all_vars = {v.name for v in tf.compat.v1.get_default_graph().as_graph_def().node}

    if verbose:
        print("Restored {}".format(",".join(initial_vars.difference(all_vars))))
        print("Existing {}".format(",".join(all_vars.difference(initial_vars))))
        print("All {}".format(",".join(all_vars)))
